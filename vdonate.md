...
## Тестирование оплаты
При желании подписаться на автора, нужно оплатить его подписку. Поэтому под оплатой подразумевается успешное пополнение баланса автора и приобретение желаемой подписки.
...
4. При переполнении экрана pop-up'ами, не уместившееся на экране можно проскроллить и увидеть ❌
   ...
   >
   > Фактический результат: При большом количестве pop-up уведомлений, экран переполняется, долистать до верхних (более старых) невозможно. 
...
5. При нажатии на колокольчик в левом сайд-баре выводятся новые уведомления и сообщение "новых уведомлений нет" в случае их отсутствия. Повторное нажатие закрывает выпадающий список ✅
...
10. Поиск работает при вводе части имени, вместо полного  ❌
    ...
    >
    > Ожидаемый результат: Список пользователей, в чьих именах присутствует часть их имени, например поиск 'они' в имени 'Тони' или сообщение о том, что таких пользователей нет.
    >
...
11. Поиск по нескольким словам, разделённым пробелом ❌
    > **[Баг]**
    > Если ввести в поисковой запрос несколько слов через пробел и нажать на кнопку "Поиск", то результат будет неверным.
    >
...
5. В режиме автора: В правом сайд-баре отображается количество донатеров, постов, донатеров за месяц; заработок за месяц, баланс и кнопка "Вывести деньги" ✅

## Тестирование страницы профиля юзера
1. В правом сайд-баре на странице пользователя отображается аватарка, ник, количество донатеров, количество постов пользователя ✅
...
16. Возможно ввести почту с разрешенными спецсимволами ✅
    > Допустимые символы: **до собаки** прописные буквы (A-Z), строчные буквы (az), цифры (0-9), точка (.), апостроф ('), тире (-), решётка (#), собака (@), процент (%), амперсанд (&), косая черта) (/) и пробелы; **после собаки** тире и точка.
...
19. Подсказка при наведении на инпут отображается ❌
    > **[БАГ]**
    > При наведении на поле для почты, появляется подсказка, которая должна указывать на структуру адреса электронной почты, но по факту не несёт смысловой нагрузки.
    >
...
1. При нажатии на кнопку *комментарий* под постом вылезают все комментарии; ❌
    ...
    >
    > Ожидаемый результат: Появление лоадера происходить не должно.
    >
    > Фактический результат: Появление комментариев сопровождатся фиктивным лоадером.
    >
...
5. Стоимость успешно валидируется при сумме, меньшей 1000000000 и большей 5 рублей:
    ...
    - При попытке написать символ, отличный от символов цифр, поле никак не заполняется; ❌
        > **[БАГ]**: Попытка вставить текст, содержащий символы, отличные от цифр, оборачивается следующей ошибкой: "Укажите цену меньше 1"
        >
    ...
    - В случае непопадания вводимого числа в указанный промежуток, вылезают следующие ошибки: "Укажите цену не больше 1000000000" и "Укажите цену меньше 1". ✅
...
> `Замечание`: Тестирование изменения и удаления авторской подписки было решено объединить, так как функционал изменения включает в себя функционал удаления подписки.
...
>
> Ожидаемый результат: Очистка всего описания и отображение в поле описание текста-уведомления об отсутствии информации о профиле
>
...
> **[БАГ]**
> При медленном интернете загрузка картинки не блокирует ввод текста, а также плохо анимирован данный сценарий
>
> Ожидаемый результат: Блокировка всех действий, кроме отмены редактирования и удаления поста
>
> Фактический результат: После начала загрузки ввод текста не блокируется, анимация кнопки сохранить нелогична (сейчас анимация загрузки, правильнее было бы сделать серым или что-то подобное), анимация загрузки на кнопке добавления картинки прыгает наверх поста
