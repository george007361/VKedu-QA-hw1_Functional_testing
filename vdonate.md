...
4. При переполнении экрана pop-up'ами, не уместившееся можно скроллить ❌
   ...
   >
   > Ожиданиемый результат: Возможность скроллинга при переполнении экрана
   > 
   > Фактический: При большом количестве pop-up'ов, экран переполняется, верхние(более старые) уведомления становятся недостигаемыми
...
9. Поисковой запрос может содержать спецсимволы (!"№;%:?*()_=+) ❌
    > **[Баг]**
    > При вводе в поисковую строку спецсимволов и сабмите формы поиска, выводится неверный результат.
    >
    > Ожидание: Сообщение о том, что пользователи не найдены или список пользователей, в чьём имени содержатся спецсимволы.
    > 
    > Реальность: Выводится список всех юзеров.
...
10. Поиск работает при вводе части имени, вместо полного  ❌
    ...
    > 
    > Реальность: Поиск завершается неверно. При существующем пользователе 'Тони', и запросе 'они', поиск выведет, что ничего не найдено.
    > ![](imgs/search_oni.png)
...
11. Поиск по нескольким словам, разделённым пробелом ❌
    ...
    > 
    > Ожидание: Список юзеров, в чьих именах присутствуют введённые слова, или сообщение о том, что ни один пользователь по запросу не найден
    > 
...
5. Email не может содержать неразрешенныхе спецсимволы (остальные) ✅
6. Возможно установить email длиной до 255 символов ❌
   > **[Баг]**
   > При вводе достаточно длинного адреса (более 64 символов), появляется сообщение об ошибке
   > 
   > Ожидание: Возможность установить email длиной до 255 символов
   > 
   > Реальность: Невозможность установить почту длинее 64 символов
   > ![](imgs/email_len_err.png)
7. Невозможно установить очень длинный email (более 255 символов) ✅
8. Возможно установить очень короткий email (a@b.c) ✅
9. Попытка адрес без собаки выводит сообщение об ошибке ✅
10. Попытка ввести почту без точки в домене выводит сообщение об ошибке✅
11. Возможно ввести почту со спецсимволами в домене ✅
    > **[Замечание]**
    > При попытке установить адрес, по типу example@domain#&^.ru запрос уходит на сервер
    > 
...
13. Подсказка при наведении на инпут отображается ✅
    > **[Замечание]**
    > При наведении на поле для почты, появляется подсказка, которая указывает на струкруту адреса.
    >
...
6. Сабмит короткого пароля (менее 5 символов) выводит сообщение об ошибке✅
7. Сабмит длинного пароля (более 30 символов) выводит сообщение об ошибке ✅
8. Сабмит при пустом поле закрывает форму, не меняя пароль ✅
9. Подсказка при наведении на инпут отображается ✅
10. pop-up при сабмите невалидных данных появляется ✅
11. Красная рамка вокруг инпута при сабмите невалидных данных отображается ✅
12. pop-up при сабмите несовпадающих паролей появляется ✅
13. Красная рамка вокруг инпута повтора пароля при сабмите несовпадающих паролей отображается ✅
...
5. Невозможно выбрать файл с расширением, отличным от изображения ❌ 
   > **[Баг]**
   > Нажать на "Выбрать файл", в модальном окне указать "Все файлы *.*", выбрать файл, отличный от изображения (напр: doc.pdf), нажать на кнопку "Сохранить"
   > 
...
6. Ограничение на размер фото 3 Мб. Попробовать загрузить более тяжелое фото ❌
   > **[Баг]**
   > Нажать на "Выбрать файл", в модальном окне указать выбрать файл  изображение (напр: ava.png) размером более 3 мб, нажать на кнопку "Сохранить"
   > 
